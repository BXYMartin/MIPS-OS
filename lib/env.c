...
u_int mkenvid(struct Env *e)
{
    ...
    /*Hint:  high bits of envid hold an increasing number. */
        do{
            next_env_id = (++next_env_id) % 48;
            r = (( next_env_id ) << (1 + LOG2NENV)) | idx;
        }while(u_id[next_env_id] == 1);
        u_id[next_env_id] = 1;
        return r;
}
...
env_init(void)
{
    ...
    for(i = NENV - 1;i >= 0;i--){
        envs[i].env_status = ENV_FREE;  // Mark environment as free
        LIST_INSERT_HEAD(&env_free_list, &envs[i], env_link);   // Insert the element
    }
                                                         ...
                                                         u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    //    va = ROUND(va, BY2PG);
        ...
        if(i == 0)
            ...
            bcopy(bin + i - offset, (void *)(page2kva(p)), MIN(BY2PG, bin_size-i+offset));
        }
 ...
 */
void
env_create_priority(u_char *binary, int size, int priority, char *name)
{ 
    struct Env *e;
    /*Step 1: Use env_alloc to alloc a new env. */
    env_alloc(&e, 0);
    strcpy(e->env_name, name);
...
void
env_create(u_char *binary, int size, char *name)
{ 
    /* Step 1: Use env_create_priority to alloc a new env with priority 1 */
    env_create_priority(binary, size, 1, name);
}
...
env_run(struct Env *e)
{
    ...
    *  context switch.You can imitate env_destroy() 's behaviors.*/
    ...
    curenv->env_runs=curenv->env_runs%4096+1;
