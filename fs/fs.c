...
void
dfs_ls(struct File *f, u_int level, u_int rec)
{
        char *p;
        struct File *dir;
    void *blk;
...
int
file_ls(struct File *f, u_int recursive)
{
    writef("File Structure Under Directory %s\n", f->f_name);
    dfs_ls(f, 0, recursive);
}