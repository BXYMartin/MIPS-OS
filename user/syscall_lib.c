...
void 
syscall_curpath(char *path, u_int op)
{
    return msyscall(SYS_curpath, path, op, 0, 0, 0);
}


int
syscall_perm(int set, int type, char *perm)
{
    return msyscall(SYS_perm, set, type, perm, 0, 0);
}